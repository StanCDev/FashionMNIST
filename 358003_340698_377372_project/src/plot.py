...
def train_test_acc_param_mlp(xtrain, ytrain, xtest, ytest, input_size, n_classes, hidden_layers, params,model,args):
    ...
    for param in params:
        ...
        AccTest.append(accuracy_fn(testPreds, ytest))

    AccTrainDrop = []
    AccTestDrop = []
    for param in params:
        if args.nn_type == "mlp":
            model = MLP(input_size, n_classes, hidden_layers,True)
        else:
            raise ValueError("Not implemented yet")
        method_obj = Trainer(model, lr=args.lr, epochs=param, batch_size=args.nn_batch_size)
        # Fit (:=train) the method on the training data
        trainPreds = method_obj.fit(xtrain, ytrain)
        AccTrainDrop.append(accuracy_fn(trainPreds, ytrain))
        testPreds = method_obj.predict(xtest)
        AccTestDrop.append(accuracy_fn(testPreds, ytest))
